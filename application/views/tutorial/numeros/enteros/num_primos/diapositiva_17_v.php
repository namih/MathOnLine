 <script src="<?php echo base_url(); ?>statics/js/tutorial/numeros/enteros/diapositiva_1.js"></script>
 <!-- <link rel="stylesheet" href="<?php echo base_url(); ?>statics/css/styles_numeros.css"> -->
<div class="container-fluid">
	<div>
		<p>Después encontramos dos factores para cada uno de los números que acabamos de escribir.</p>
		<!--Colocar imagen -->
		<p> Completa lo que falta </p>
		<!--Imagen de arbol -->
		<p>Seguimos este procedimiento <b>hasta que todos los factores sean primos.</b></p>
		<p>Asi podemos escribir el número como <b>producto de primos.</b></p>
		<input class="input-sm"  type="text"  id="5" value=""/><label> X </label>
		<input class="input-sm"  type="text"  id="6" value=""/><label> X </label>
		<input class="input-sm"  type="text"  id="7" value=""/><label> X </label>
		<input class="input-sm"  type="text"  id="8" value=""/><label> =90 </label>
	</div>
	<br/><br/><br />
	<input type="button" class="btn btn-success btn-sm" onclick="verificar();" value="VERIFICAR" />
  </div>		
