              ...
              <ul class="nav navbar-nav navbar-right">
                <?php foreach ($menu_user as $idMenu => $datosMenu) {
                    if(empty($datosMenu['submenu'])){//si no hay submenu se pinta un link sencillo ?>
                      <li><a href="<?php if(trim($datosMenu['href'])==''){ echo '#'; }else { echo trim($datosMenu['href']); };?>" <?php if(trim($datosMenu['onclick'])!=''){ echo 'onclick="loadContent(\''.base_url().$datosMenu['onclick'].'\');"'; }  ?> ><?php echo $datosMenu['nombre']; ?></a></li>
                    <?php }else{ ?>
                      <li class="dropdown">
